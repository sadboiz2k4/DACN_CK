...
const BANKS = [
  'VCB', 'TCB', 'MB', 'ACB', 'BIDV', 'CTG', 'VPB', 'TPB', 'VIB', 'MSB',
  'STB', 'HDB', 'OCB', 'SHB', 'EIB', 'LPB', 'VAB', 'BAB',
]
...
function GroupModal({ user, onClose, onSaved }) {
  ...
  const [members, setMembers] = useState([
    { ...emptyMember, displayName: user?.fullName || '', email: user?.email || '', currentUser: true },
    { ...emptyMember },
    { ...emptyMember },
  ])
    ...
    try {
      const payload = {
        ...
        members: members
          ...
          .map(m => ({ ...m, bankCode: m.bankCode?.trim().toUpperCase() })),
      }
              ...
              <div key={idx} className="rounded-lg border border-gray-100 p-3">
                <div className="grid grid-cols-1 gap-3 md:grid-cols-5">
                  ...
                  <label className="flex items-center gap-2 text-sm text-gray-600">
                    <input
                      type="checkbox"
                      checked={member.currentUser}
                      onChange={e => updateMember(idx, { currentUser: e.target.checked })}
                    />
                    Là tôi
                  </label>
                </div>
...
function MemberModal({ group, member, onClose, onSaved }) {
  ...
  const submit = async (e) => {
    ...
    setSaving(true)
    try {
      const payload = { ...form, bankCode: form.bankCode?.trim().toUpperCase() }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <form onSubmit={submit} className="w-full max-w-md rounded-xl bg-white p-5 shadow-xl">
        ...
        <div className="space-y-3">
          ...
          <label className="flex items-center gap-2 text-sm text-gray-600">
            <input type="checkbox" checked={!!form.currentUser} onChange={e => setForm(f => ({ ...f, currentUser: e.target.checked }))} />
            Đại diện cho tài khoản của tôi
          </label>
        </div>
        <div className="mt-5 flex justify-end gap-3">
          <button type="button" className="btn-secondary" onClick={onClose}>Hủy</button>
...
function BillModal({ group, wallets, selectedWalletId, onClose, onSaved }) {
  ...
  const applyEqual = () => {
    ...
    const base = Math.floor(total / includedRows.length)
    let used = 0
    setRows(prev => prev.map(row => {
      if (!row.included) return { ...row, shareAmount: '', sharePercent: '' }
      const isLast = includedRows[includedRows.length - 1].memberId === row.memberId
      const share = isLast ? total - used : base
      used += share
      return { ...row, shareAmount: String(share), sharePercent: String((share / total * 100).toFixed(2)) }
    }))
    setSplitMode('EQUAL')
  }

  const applyEqualPercent = () => {
    if (includedRows.length === 0) return
    const percent = Number((100 / includedRows.length).toFixed(2))
    setRows(prev => prev.map(row => row.included ? { ...row, sharePercent: String(percent) } : row))
  ...
  const applyPercent = () => {
    ...
    setRows(prev => prev.map(row => {
      if (!row.included) return { ...row, shareAmount: '' }
      const isLast = active[active.length - 1].memberId === row.memberId
      const share = isLast ? total - used : Math.round(total * asNumber(row.sharePercent) / 100)
      used += share
      return { ...row, shareAmount: String(share) }
    }))
    setSplitMode('PERCENT')
  }
    ...
    try {
      await createSplitBill(group.id, {
        ...
        participants: rows.map(row => ({
          ...
          sharePercent: row.sharePercent ? Number(row.sharePercent) : null,
        })),
      })
        ...
        <form onSubmit={submit} className="max-h-[80vh] overflow-y-auto p-5">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
            ...
            <div>
              ...
              <input type="number" className="input" value={form.totalAmount} onChange={e => setForm(f => ({ ...f, totalAmount: e.target.value }))} required min="1" />
            </div>
            <div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            <button type="button" className="btn-secondary py-1.5 text-sm" onClick={applyEqualPercent}>Điền % đều</button>
            <button type="button" className="btn-secondary py-1.5 text-sm" onClick={applyPercent}>Áp dụng %</button>
                ...
                {rows.map(row => {
                  ...
                  return (
                    <tr key={row.memberId}>
                      ...
                      <td className="px-3 py-2">
                        <input type="number" className="input" value={row.paidAmount} onChange={e => updateRow(row.memberId, { paidAmount: e.target.value })} min="0" />
                      </td>
                      <td className="px-3 py-2">
                        <input type="number" className="input" value={row.sharePercent} onChange={e => updateRow(row.memberId, { sharePercent: e.target.value })} min="0" step="0.01" disabled={!row.included} />
                      </td>
                      <td className="px-3 py-2">
                        <input type="number" className="input" value={row.shareAmount} onChange={e => { updateRow(row.memberId, { shareAmount: e.target.value }); setSplitMode('AMOUNT') }} min="0" disabled={!row.included} />
                      </td>
...
function SettlementCard({ settlement, selectedWalletId, onChanged }) {
  ...
  const canMarkPaid = settlement.status === 'PENDING'
  const canConfirm = settlement.status !== 'CONFIRMED'
